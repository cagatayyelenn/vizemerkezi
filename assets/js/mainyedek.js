document.addEventListener('DOMContentLoaded', () => {
    // 1. Initialize Global Components (Modal)
    initModal();

    // 2. Handle Dynamic Page Content
    handleDynamicContent();

    // 3. Initialize Dropdowns (if present)
    initDropdowns();
});
                ...
                <div class="max-h-[90vh] overflow-y-auto">
                    <div class="relative">
                        <div id="modalInnerForm" class="bg-white p-8 md:p-12 rounded-[2.5rem] shadow-2xl border border-blue-50">
                            <div class="mb-10">
                                <h3 class="text-3xl font-bold text-blue-950 mb-3">Vize Başvuru Formu</h3>
...
async function handleDynamicContent() {
    ...
    try {
        // Correct path to data.json based on current URL depth
        // If we are deep (/ulke/almanya), we need to go up one level
        let fetchUrl = 'assets/data/data.json';
        if (path.includes('/ulke/') || path.includes('/hizmet/')) {
            fetchUrl = '../assets/data/data.json';
        }
        ...
        if (path.includes('ulke-detay') || path.includes('/ulke/')) {
            loadCountryData(code, data.countries);
        } else if (path.includes('hizmet-detay') || path.includes('/hizmet/')) {
...
function loadCountryData(code, countries) {
    ...
    // We can expand this heavily based on the exact IDs we place in ulke-detay.html
}
